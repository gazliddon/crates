use emucore::mem::{MemoryIO, MemResult};
...
pub fn diss_operand<M: MemoryIO>(mem: &M, addr: usize, ins: &InstructionInfo) -> MemResult<String> {
    use super::AddrModeEnum::*;
    let text = match ins.addr_mode {
        AccA => "A".to_owned(),
        AccB => "B".to_owned(),
        Immediate => {
            let b = mem.inspect_byte(addr)?;
            format!("#0x{b:02x}")
        }

        Immediate16 => {
            let w = mem.inspect_word(addr)?;
            format!("#0x{w:04x}")
        }
        Direct => {
            let b = mem.inspect_byte(addr)?;
